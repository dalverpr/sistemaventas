...
<body>
    <table style="width: 100%">
        <tr>
            <td colspan="13" class="title"><strong>Reporte de Beneficios</strong></td>
        </tr>
        <tr>
    ...
    <table style="width: 100%" class="table-records">
        <thead>
            ...
            <tr>
                <th>Saldo Final - {{ $ultimoDiaMesAnterior }}</th>
                <th>Compras - {{ $ultimoDiaMes }}</th>
                <th>Gastos - {{ $ultimoDiaMes }}</th>
                <th>Ingresos - {{ $ultimoDiaMes }}</th>
                <th>Ventas - {{ $ultimoDiaMes }}</th>
                <th>Costo - {{ $ultimoDiaMes }}</th>
                <th>Beneficio</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                <td align="right">{{ $profit->fbpm }}</td>
                <td align="right">{{ $profit->purchases }}</td>
                <td align="right">{{ $profit->expenses }}</td>
                <td align="right">{{ $profit->income }}</td>
                <td align="right">{{ $profit->sales }}</td>
                <td align="right">{{ $profit->fbam }}</td>
                <td align="right">{{ number_format(abs($profit->sales - $profit->fbam), 2, '.', '') }}</td>
            </tr>
    ...
    <table style="width: 100%" class="table-records">
        ...
        <tbody>
            ...
            @foreach ($records as $key => $row)
                @php
                    //dd($row['item']['purchase_unit_price']);
                    $total_line = $row['stock'] * $row['item']['purchase_unit_price'];
                    $profit = $row['item']['sale_unit_price'] - $row['item']['purchase_unit_price'];
                    $total += $total_line;
                    $saleCost = $row['item']['sale_unit_price'] * $row['stock'];
                    $total_profit += $profit;
                    $total_all_profit += $profit * $row['stock'];
                    $profit = number_format($profit, 2, '.', '');
                    $currency = $row['item']['currency_type_id'];
                    $total_purchase_unit_price += $row['item']['purchase_unit_price'];
                    $total_sale_unit_price += $row['item']['sale_unit_price'];
                ...
                <tr>
                    ...
                    <td align="right">{{ $row['item']['purchase_unit_price'] }}</td>
                    <td align="right">{{ $total_line }}</td>
                    <td align="right">{{ $row['item']['sale_unit_price'] }}</td>
